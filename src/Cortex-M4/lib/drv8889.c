 ...
 * 
 *          * SPI2:
 *              - SCK (B10) -- gpioset gpiochip1 10=1
 *              - MOSI (C3) -- gpioset gpiochip2 3=1
 *              - MISO (I2) -- gpioset gpiochip8 2=1
 * 
 *          * M1:
 *              - nSleep (I3) -- gpioset gpiochip8 3=1
 *              - E_M (C12) -- gpioset gpiochip2 12=1
 *              - DIR (D2) -- gpioset gpiochip3 2=1
 *              - STEP (C11) -- gpioset gpiochip2 11=1
 *              - CS (C10) -- gpioset gpiochip2 10=1
 *              - nFault (C8) -- gpioset gpiochip2 8=1
 * 
 *          * M2:
 *              - nSleep (E13)
 *              - E_M (D3)
...
Set_CS_Pin (uint8_t pin)
{
    ...
    switch (pin)
    {
        case CS_1:
            ...
            HAL_GPIO_WritePin(GPIOI, GPIO_PIN_3, GPIO_PIN_SET);     // PI3 - nSleep_M1 
            HAL_GPIO_WritePin(GPIOC, GPIO_PIN_12, GPIO_PIN_SET);    // PC12 - EM_M1
            HAL_GPIO_WritePin(GPIOD, GPIO_PIN_2, GPIO_PIN_RESET);   // PD2  - DIR_M1
            HAL_GPIO_WritePin(GPIOC, GPIO_PIN_11, GPIO_PIN_RESET);  // PC11 - STEP_M1            
            HAL_GPIO_WritePin(GPIOC, GPIO_PIN_10, GPIO_PIN_SET);    // PC10 - CS_M1
            break;
        case CS_2:
...
{
    applySettings(ctrl1, ctrl2, ctrl3, ctrl4, ctrl5, ctrl6, ctrl7);
}
...
writeReg (uint8_t addr, uint8_t data)
{
    ...
    if (HAL_SPI_Transmit(&hspi2, (addr & 0b11111) << 1, 1, 100000) != HAL_OK)
		Error_Handler();
...
void writeCachedReg(uint8_t addr)
{
    uint8_t *cachedReg = &addr;
    if (!cachedReg)
        return;
    writeReg(addr, *cachedReg);
}
...
void 
applySettings (uint8_t ctrl1, uint8_t ctrl2, uint8_t ctrl3, uint8_t ctrl4, uint8_t ctrl5, uint8_t ctrl6, uint8_t ctrl7)
{
    writeCachedReg(ctrl1);
    writeCachedReg(ctrl3);
    writeCachedReg(ctrl4);
    writeCachedReg(ctrl5);
    writeCachedReg(ctrl6);
    writeCachedReg(ctrl7);

    // CTRL2 is written last because it contains the EN_OUT bit, and we want to
    // try to have all the other settings correct first.
    writeCachedReg(ctrl2);
}
