...
#include "ms8607.h"
  ...
  */
#include "i2c_linker.h"  // I2C linker
// #include "math.h"
...
static enum ms8607_status hsensor_reset(uint8_t hi2c);
static bool hsensor_is_connected(uint8_t hi2c);
static enum ms8607_status hsensor_write_command(uint8_t hi2c, uint8_t);
...
static enum ms8607_status hsensor_read_user_register(uint8_t hi2c, uint8_t *);
static enum ms8607_status hsensor_write_user_register(uint8_t hi2c, uint8_t);
static enum ms8607_status hsensor_humidity_conversion_and_read_adc(uint8_t hi2c, uint16_t *);
static enum ms8607_status hsensor_read_relative_humidity(uint8_t hi2c, float *);

// Pressure sensor functions
static enum ms8607_status psensor_reset(uint8_t hi2c);
static bool psensor_is_connected(uint8_t hi2c);
static enum ms8607_status psensor_write_command(uint8_t hi2c, uint8_t);
static enum ms8607_status psensor_read_eeprom_coeff(uint8_t hi2c, uint8_t, uint16_t*);
static enum ms8607_status psensor_read_eeprom(uint8_t hi2c);
static enum ms8607_status psensor_conversion_and_read_adc(uint8_t hi2c, uint8_t, uint32_t *);
static bool psensor_crc_check (uint16_t *n_prom, uint8_t crc);
enum ms8607_status psensor_read_pressure_and_temperature(uint8_t hi2c, float *, float *);
 ...
 *       - false : Device is not acknowledging I2C address
  */
bool ms8607_is_connected(uint8_t hi2c)
{
	return (hsensor_is_connected(hi2c) && psensor_is_connected(hi2c));
}
 ...
 */
enum ms8607_status  ms8607_reset(uint8_t hi2c)
{
	enum ms8607_status status;
	
	status = hsensor_reset(hi2c);
	if( status != ms8607_status_ok)
		return status;
	status = psensor_reset(hi2c);
	if( status != ms8607_status_ok)
		return status;
 ...
 */
enum ms8607_status ms8607_set_humidity_resolution(uint8_t hi2c, enum ms8607_humidity_resolution res)
{
	enum ms8607_status status;
	...
	status = hsensor_read_user_register(hi2c, &reg_value);
	if( status != ms8607_status_ok )
		return status;
	...
	status = hsensor_write_user_register(hi2c, reg_value);
...
/*
PHT_Values
ms8607_read_temperature_pressure_humidity (PHT_Values pht)
{
	float *p = NULL;
	float *h = NULL;
	float *t = NULL;
	
	pht.status = psensor_read_pressure_and_temperature(t, p);
	if(pht.status != ms8607_status_ok)
		return pht;

	pht.status = hsensor_read_relative_humidity(h);
	if(pht.status != ms8607_status_ok)
		return pht;
		
	pht.temperature = (int16_t)(*t * TEMP_FACTOR);
	pht.pressure = (int16_t)(*p * PRESSURE_FACTOR);
	pht.humidity = (int16_t)(*h * HUMIDITY_FACTOR);
		
		
	return pht;
}*/

enum ms8607_status ms8607_read_temperature_pressure_humidity(uint8_t hi2c, float *t, float *p, float *h)
{
	enum ms8607_status status;
	
	status = psensor_read_pressure_and_temperature(hi2c, t, p);
	if (status != ms8607_status_ok)
		return status;

	status = hsensor_read_relative_humidity(hi2c, h);
	if (status != ms8607_status_ok)
		return status;
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms8607_status ms8607_get_battery_status(uint8_t hi2c, enum ms8607_battery_status *bat)
{
	enum ms8607_status	status;
	uint8_t reg_value;
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms8607_status ms8607_enable_heater(uint8_t hi2c)
{
	enum ms8607_status status;
	uint8_t reg_value;
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms8607_status ms8607_disable_heater(uint8_t hi2c)
{
	enum ms8607_status status;
	uint8_t reg_value;
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms8607_status ms8607_get_heater_status(uint8_t hi2c, enum ms8607_heater_status *heater)
{
	enum ms8607_status status;
	uint8_t reg_value;
 ...
 *       - false : Device is present
 *       - true : Device is not acknowledging I2C address
  */
bool hsensor_is_connected(uint8_t hi2c)
{
	...
	// Do the transfer
	i2c_status = i2c_master_write_packet_wait(hi2c, &transfer);
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms8607_status  hsensor_reset(uint8_t hi2c)
{
	enum ms8607_status status;
	
	status = hsensor_write_command(hi2c, HSENSOR_RESET_COMMAND);
	if( status != ms8607_status_ok )
		return status;
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms8607_status hsensor_write_command(uint8_t hi2c, uint8_t cmd)
{
	enum status_code i2c_status;
	uint8_t data[1];
...
enum ms8607_status hsensor_write_command_no_stop( uint8_t cmd)
{
	...
	/* Do the transfer */
	i2c_status = i2c_master_write_packet_wait_no_stop(0, &transfer);
	if( i2c_status == STATUS_ERR_OVERFLOW )
		return ms8607_status_no_i2c_acknowledge;
	if( i2c_status != STATUS_OK)
		return ms8607_status_i2c_transfer_error;
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms8607_status hsensor_read_user_register(uint8_t hi2c, uint8_t *value)
{
	enum ms8607_status status;
	enum status_code i2c_status;
	uint8_t buffer[1];
	...
	status = hsensor_write_command(hi2c, HSENSOR_READ_USER_REG_COMMAND);
	if( status != ms8607_status_ok )
		return status;
	
	i2c_status = i2c_master_read_packet_wait(hi2c, &read_transfer);
	if( i2c_status == STATUS_ERR_OVERFLOW )
		return ms8607_status_no_i2c_acknowledge;
	if( i2c_status != STATUS_OK)
		return ms8607_status_i2c_transfer_error;
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms8607_status hsensor_write_user_register(uint8_t hi2c, uint8_t value)
{
	enum ms8607_status status;
	enum status_code i2c_status;
	uint8_t reg;
	...
	status = hsensor_read_user_register(hi2c, &reg);
	if( status != ms8607_status_ok )
		return status;
 ...
 *       - ms8607_status_crc_error : CRC check error
 */
enum ms8607_status hsensor_humidity_conversion_and_read_adc(uint8_t hi2c, uint16_t *adc)
{
	...
	if( hsensor_i2c_master_mode == ms8607_i2c_hold) {
		...
		//delay(hsensor_conversion_time / 1000); // 16000 / 1000
		_delay_ms(16);  //////////////////////
	}
	*/
	
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	//buffer[0] = HSENSOR_READ_HUMIDITY_WO_HOLD_COMMAND;
	//tw_master_transmit(HSENSOR_ADDR, buffer, 1, false); // Write + stop
	
	status = hsensor_write_command(hi2c, HSENSOR_READ_HUMIDITY_WO_HOLD_COMMAND);
	HAL_Delay(16);  /* hsensor_conversion_time / 1000 - 16000 / 1000 */
		
	//if (tw_master_receive(HSENSOR_ADDR, buffer, sizeof(buffer)))
	//	return ms8607_status_i2c_transfer_error;
	////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 *       - ms8607_status_crc_error : CRC check error
 */
enum ms8607_status hsensor_read_relative_humidity(uint8_t hi2c, float *humidity)
{
	enum ms8607_status	status;
	uint16_t adc;
	
	status = hsensor_humidity_conversion_and_read_adc(hi2c, &adc);
	if( status != ms8607_status_ok)
		return status;
 ...
 *       - false : Device is not acknowledging I2C address
  */
bool psensor_is_connected(uint8_t hi2c)
{
	enum status_code i2c_status;
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms8607_status  psensor_reset(uint8_t hi2c)
{
	return psensor_write_command(hi2c, PSENSOR_RESET_COMMAND);
}
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms8607_status psensor_write_command(uint8_t hi2c, uint8_t cmd)
{
	enum status_code i2c_status;
	uint8_t data[1];
 ...
 */
enum ms8607_status psensor_read_eeprom_coeff(uint8_t hi2c, uint8_t command, uint16_t *coeff)
{
	enum ms8607_status status;
	enum status_code i2c_status;
	uint8_t buffer[2];
	...
	status = psensor_write_command(hi2c, command);
	if(status != ms8607_status_ok)
		return status;
 ...
 *       - ms8607_status_crc_error : CRC check error on the coefficients
 */
enum ms8607_status psensor_read_eeprom(uint8_t hi2c)
{
	enum ms8607_status status;
	uint8_t i;
	...
	{
		status = psensor_read_eeprom_coeff(hi2c, PROM_ADDRESS_READ_ADDRESS_0 + i*2, eeprom_coeff+i);
		if(status != ms8607_status_ok)
			return status;
	}
 ...
 *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
static enum ms8607_status psensor_conversion_and_read_adc(uint8_t hi2c, uint8_t cmd, uint32_t *adc)
{
	enum ms8607_status status;
	enum status_code i2c_status;
	uint8_t buffer[3];
	...
	status = psensor_write_command(hi2c, cmd);
	// 20ms wait for conversion
	HAL_Delay(20);
	//delay(psensor_conversion_time[ (cmd & PSENSOR_CONVERSION_OSR_MASK)/2 ]/1000 );
	if( status != ms8607_status_ok)
		return status;

	// Send the read command
	status = psensor_write_command(hi2c, PSENSOR_READ_ADC);
	if( status != ms8607_status_ok)
		return status;
 ...
 *       - ms8607_status_crc_error : CRC check error on the coefficients
 */
enum ms8607_status psensor_read_pressure_and_temperature(uint8_t hi2c, float *temperature, float *pressure)
{
	enum ms8607_status status = ms8607_status_ok;
	...
	status = psensor_read_eeprom(hi2c);
	if (status != ms8607_status_ok)
		return status;
	...
	status = psensor_conversion_and_read_adc(hi2c, cmd, &adc_temperature);
	if( status != ms8607_status_ok)
		return status;
	...
	status = psensor_conversion_and_read_adc(hi2c, cmd, &adc_pressure);
	if( status != ms8607_status_ok)
		return status;
